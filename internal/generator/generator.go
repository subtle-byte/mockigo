...
func Generate(cnf *config.InitializedConfig, pkgDirs map[DirPath]Interfaces) error {
	...
	for _, pkg := range pkgs {
		...
		var mockDir string
		mockPkgName := fmt.Sprintf("%s%s%s", cnf.PkgPrefix, pkg.Name, cnf.PkgPostfix)
		if cnf.InDir {
			mockDir = filepath.Join(pkgDir, mockPkgName)
		} else {
			inRoot, err := filepath.Rel(cnf.RootDir, pkgDir)
			if err != nil {
				log.Printf("ERROR: get rel path of package dir %s in root dir: %v\n", pkgDir, err)
				continue
			}
			mockDir = filepath.Join(cnf.MocksDir, inRoot)
		}
		interfacesSelector, ok := pkgDirs[pkgDir]
		if !ok {
			panic(fmt.Sprintf("%#v %q", pkgDirs, pkgDir))
		}
		err = generateForPackage(pkg.Types, mockDir, mockPkgName, interfacesSelector)
		if err != nil {
...
func generateForPackage(pkg *types.Package, mockDir, mockPkgName string, interfaces Interfaces) error {
	for _, ident := range pkg.Scope().Names() {
		...
		generated, err := generateForInterface(mockPkgName, ident, typeParams, methods)
		if err != nil {
			return fmt.Errorf("generate for interface (interface = %s, package = %s): %w", ident, pkg.Path(), err)
		}
		fileToSave := filepath.Join(mockDir, string_util.CamelToSnake(ident)+".go")
		err = writeToFile(generated, fileToSave)
		if err != nil {
			return fmt.Errorf("writing file to output (file = %s, interface = %s, package = %s): %w", fileToSave, ident, pkg.Path(), err)
		}
	}
	return nil
}
...
func generateForInterface(mockPkgName, interfaceName string, typeParams *types.TypeParamList, methods []*types.Func) ([]byte, error) {
	buf := &bytes.Buffer{}
	w := &writer{buf: buf}

	w.Println("// Code generated by mockigo. DO NOT EDIT.")
	w.Println()
	w.Println("package " + mockPkgName)
