...
type GenericInterface[T any, B some_interface.SomeInterface, G ~int|float32] struct {
	mock *mock.Mock
}

func NewGenericInterface[T any, B some_interface.SomeInterface, G ~int|float32](t mock.Testing) *GenericInterface[T, B, G] {
	t.Helper()
	return &GenericInterface[T, B, G]{mock: mock.NewMock(t)}
}

type _GenericInterface_Expecter[T any, B some_interface.SomeInterface, G ~int|float32] struct {
	mock *mock.Mock
}
...
type _GenericInterface_SomeMethod_Call[T any, B some_interface.SomeInterface, G ~int|float32] struct {
	*mock.Call
}
